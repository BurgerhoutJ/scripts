...
if (-not (Test-Path "$($env:ProgramData)\ThePeskyGhosts\ResetMDMDevice"))
{
    Mkdir "$($env:ProgramData)\ThePeskyGhosts\ResetMDMDevice"
}
Set-Content -Path "$($env:ProgramData)\ThePeskyGhosts\ResetMDMDevice\Reset-MDMWipeDevice.ps1.tag" -Value "Installed"
...
$form.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#2D1B69")
